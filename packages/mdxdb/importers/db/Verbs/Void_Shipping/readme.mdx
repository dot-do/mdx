---
id: Void_Shipping
name: Void Shipping
title: Void Shipping
description: Denotes the cancellation of a shipment
collection: Verbs
source: gs1
type: BusinessStep
'@context': 'https://schema.org'
'@type': Action
category: supply-chain
cbvType: bizStep
url: 'https://ref.gs1.org/cbv/BizStep-VoidShipping'
aiEnriched: false
enrichmentVersion: '1.0'
synonyms: []
antonyms: []
relatedVerbs: []
usedInIndustries: []
usedInProcesses: []
examples: []
prerequisites: []
outcomes: []
tags: []
implementations: []
standards:
  - name: GS1 CBV 2.0
    url: 'https://ref.gs1.org/standards/cbv/'
    version: '2.0'
  - name: EPCIS 2.0
    url: 'https://ref.gs1.org/standards/epcis/'
    version: '2.0'
createdAt: '2025-10-04T18:27:45.176Z'
updatedAt: '2025-10-04T18:27:45.176Z'
---
# Void Shipping

**GS1 Business Step**

## Description

Denotes the cancellation of a shipment

## Category

Supply Chain Operations

## Usage

This business step is part of the GS1 Core Business Vocabulary (CBV) 2.0 standard. It is used in EPCIS events to indicate the business context of supply chain activities.

## Related Standards

- **GS1 CBV 2.0**: [Core Business Vocabulary](https://ref.gs1.org/standards/cbv/)
- **EPCIS 2.0**: [Event Processing Standard](https://ref.gs1.org/standards/epcis/)

## References

- [Official GS1 Definition](https://ref.gs1.org/cbv/BizStep-VoidShipping)
- [GS1 Standards Documentation](https://www.gs1.org/standards)
