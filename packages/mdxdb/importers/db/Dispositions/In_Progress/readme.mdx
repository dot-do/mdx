---
id: In_Progress
name: In Progress
title: In Progress
description: Business process is in progress
collection: Dispositions
source: gs1
type: ObjectState
'@context': 'https://schema.org'
'@type': State
category: object-state
cbvType: disposition
url: 'https://ref.gs1.org/cbv/Disp-InProgress'
aiEnriched: false
enrichmentVersion: '1.0'
transitionsFrom: []
transitionsTo: []
usedInIndustries: []
appliesTo: []
examples: []
tags: []
implications: []
standards:
  - name: GS1 CBV 2.0
    url: 'https://ref.gs1.org/standards/cbv/'
    version: '2.0'
  - name: EPCIS 2.0
    url: 'https://ref.gs1.org/standards/epcis/'
    version: '2.0'
createdAt: '2025-10-04T18:27:45.218Z'
updatedAt: '2025-10-04T18:27:45.218Z'
---
# In Progress

**GS1 Business Object Disposition**

## Description

Business process is in progress

## Category

Object State Management

## Usage

This disposition value is part of the GS1 Core Business Vocabulary (CBV) 2.0 standard. It is used in EPCIS events to indicate the business state or condition of objects in the supply chain.

## Related Standards

- **GS1 CBV 2.0**: [Core Business Vocabulary](https://ref.gs1.org/standards/cbv/)
- **EPCIS 2.0**: [Event Processing Standard](https://ref.gs1.org/standards/epcis/)

## References

- [Official GS1 Definition](https://ref.gs1.org/cbv/Disp-InProgress)
- [GS1 Standards Documentation](https://www.gs1.org/standards)
