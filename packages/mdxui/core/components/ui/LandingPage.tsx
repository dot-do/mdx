...
export const Hero = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & HeroSection>(
  (
    ...
    <section ref={ref} className={cn('relative w-full py-12 md:py-24 lg:py-32 overflow-hidden', className)} {...props}>
      <div className='container px-4 md:px-6 mx-auto'>
        <div className='grid gap-6 lg:grid-cols-2 lg:gap-12 items-center'>
          <div className='flex flex-col justify-center space-y-4'>
            {badge && <div className='inline-block rounded-full bg-muted px-3 py-1 text-sm'>{badge}</div>}
            <motion.h1
            ...
            <div className='flex flex-col sm:flex-row gap-3'>
              <a href={primaryActionLink} className={cn(buttonVariants({ size: 'lg' }), 'rounded-full')}>
                {primaryActionText}
              </a>
              {secondaryActionText && secondaryActionLink && (
                <a href={secondaryActionLink} className={cn(buttonVariants({ variant: 'outline', size: 'lg' }), 'rounded-full')}>
                  {secondaryActionText}
                </a>
              )}
            </div>
...
export const Problem = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & ProblemSection>(
  ({ className, headline, description, points, imageUrl, imageAlt, badge, ...props }, ref) => (
    <section ref={ref} className={cn('w-full py-12 md:py-24 lg:py-32 bg-muted/50', className)} {...props}>
      <div className='container px-4 md:px-6 mx-auto'>
        <div className='grid gap-6 lg:grid-cols-2 lg:gap-12 items-center'>
          {imageUrl && (
            <div className='order-2 lg:order-1'>
              <img src={imageUrl} alt={imageAlt || headline || 'Problem illustration'} className='mx-auto rounded-lg object-cover' width={500} height={500} />
            </div>
          )}
          <div className={cn('space-y-4', imageUrl ? 'order-1 lg:order-2' : '')}>
            {badge && <div className='inline-block rounded-full bg-muted px-3 py-1 text-sm'>{badge}</div>}
            {headline && <h2 className='text-3xl font-bold tracking-tighter sm:text-4xl md:text-5xl'>{headline}</h2>}
      ...
      <CardHeader>
        {iconUrl && (
          <div className='mb-2 flex h-10 w-10 items-center justify-center rounded-full bg-muted'>
            <img src={iconUrl} alt={iconAlt || title} className='h-5 w-5' />
          </div>
        )}
...
export const Features = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & FeaturesSection>(
  ({ className, headline, description, features, badge, primaryActionText, primaryActionLink, secondaryActionText, secondaryActionLink, ...props }, ref) => (
    <section ref={ref} className={cn('w-full py-12 md:py-24 lg:py-32', className)} {...props}>
      <div className='container px-4 md:px-6 mx-auto'>
...
const PricingCard = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & PricingPlan>(
  ({ className, name, price, description, features, ctaText, ctaLink, featured, ...props }, ref) => (
    <Card ref={ref} className={cn('flex flex-col', featured ? 'border-primary shadow-lg' : '', className)} {...props}>
      <CardHeader>
        {featured && <div className='inline-block rounded-full bg-primary px-3 py-1 text-sm text-primary-foreground mb-2'>Popular</div>}
  ...
  ({ className, headline, description, plans, note, badge, ...props }, ref) => (
    <section ref={ref} className={cn('w-full py-12 md:py-24 lg:py-32', className)} {...props}>
      <div className='container px-4 md:px-6 mx-auto'>
        ...
        {note && <div className='mt-8 text-center text-sm text-muted-foreground'>{note}</div>}
      </div>
    </section>
  ),
)
    ...
    <Card ref={ref} className={cn('h-full', className)} {...props}>
      <CardContent className='p-6 flex flex-col h-full'>
        ...
        <div className='flex items-center gap-4 mt-4 pt-4 border-t'>
          {authorImageUrl && <img src={authorImageUrl} alt={authorImageAlt || author} className='rounded-full h-10 w-10 object-cover' />}
          <div>
            <p className='font-medium'>{author}</p>
            {authorTitle && <p className='text-sm text-muted-foreground'>{authorTitle}</p>}
          </div>
        </div>
      </CardContent>
    </Card>
...
export const Testimonials = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & TestimonialsSection>(
  ({ className, headline, description, testimonials, logos, badge, ...props }, ref) => (
    <section ref={ref} className={cn('w-full py-12 md:py-24 lg:py-32 bg-muted/30', className)} {...props}>
      <div className='container px-4 md:px-6 mx-auto'>
        <div className='flex flex-col items-center justify-center space-y-4 text-center'>
          {badge && <div className='inline-block rounded-full bg-muted px-3 py-1 text-sm'>{badge}</div>}
          {headline && <h2 className='text-3xl font-bold tracking-tighter sm:text-4xl md:text-5xl'>{headline}</h2>}
...
const FAQAccordion = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & FAQItem>(({ className, question, answer, ...props }, ref) => {
  const [isOpen, setIsOpen] = React.useState(false)

  return (
    <div ref={ref} className={cn('border-b', className)} {...props}>
      <button onClick={() => setIsOpen(!isOpen)} className='flex w-full items-center justify-between py-4 text-left font-medium transition-all'>
        <span>{question}</span>
        <svg
          xmlns='http://www.w3.org/2000/svg'
          width='24'
      ...
      <div className={cn('grid transition-all', isOpen ? 'grid-rows-[1fr] opacity-100 pb-4' : 'grid-rows-[0fr] opacity-0')}>
...
export const CallToAction = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & CallToActionSection>(
  ({ className, headline, description, primaryActionText, primaryActionLink, secondaryActionText, secondaryActionLink, badge, ...props }, ref) => (
    <section ref={ref} className={cn('w-full py-12 md:py-24 lg:py-32 bg-primary text-primary-foreground', className)} {...props}>
      <div className='container px-4 md:px-6 mx-auto'>
        <div className='flex flex-col items-center justify-center space-y-4 text-center'>
          {badge && <div className='inline-block rounded-full bg-primary-foreground/20 px-3 py-1 text-sm text-primary-foreground'>{badge}</div>}
          <motion.h2
          ...
          {description && <p className='max-w-[700px] text-primary-foreground/80 md:text-xl'>{description}</p>}
    ...
    <Card ref={ref} className={cn('overflow-hidden', className)} {...props}>
      {photoUrl && (
        <div className='aspect-square overflow-hidden'>
          <img src={photoUrl} alt={photoAlt || name} className='object-cover w-full h-full transition-transform hover:scale-105' />
        </div>
      )}
