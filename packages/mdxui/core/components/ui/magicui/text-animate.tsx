...
export const TextAnimate = React.forwardRef<HTMLDivElement, TextAnimateProps>(
  ({ text, delay = 0, duration = 0.05, className, textClassName, staggerChildren = 0.1, once = true, animateOnView = true, id, style }, ref) => {
          ...
          {text.split(' ').map((word, wordIndex) => (
            <span key={`word-${wordIndex}`} className='mr-1.5 mt-1.5 inline-block'>
              {word.split('').map((char, charIndex) => (
                <motion.span key={`char-${charIndex}`} className='inline-block' variants={childVariants}>
                  {char}
                </motion.span>
              ))}
            </span>
