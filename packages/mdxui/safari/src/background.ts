...
async function injectShiki(tabId: number): Promise<void> {
  await chrome.scripting.executeScript({
    target: { tabId },
    world: 'MAIN',
    func: () => {
      // Check if Shiki is already loaded
      if (window.__shikiLoaded) {
        console.log('Main world: Shiki already loaded, dispatching ready event')
        window.dispatchEvent(new CustomEvent('shiki-ready'))
        return
      }

      console.log('Main world: Starting Shiki injection')

      // Try jsDelivr CDN which handles WASM dependencies better
      const script = document.createElement('script')
      script.type = 'module'
      script.textContent = `
        (async () => {
          try {
            console.log('Main world: Loading Shiki from jsDelivr CDN');
            
            // Use jsDelivr which handles WASM dependencies
            const { createHighlighter } = await import('https://cdn.jsdelivr.net/npm/shiki@3.8.0/+esm');
            
            console.log('Main world: Creating Shiki highlighter');
            ...
            window.__shikiHighlighter = shikiHighlighter;
            window.__shikiLoaded = true;
            
            console.log('Main world: Shiki loaded successfully');
            
            // Set up event listener for highlighting requests
            window.addEventListener('shiki-highlight-request', async (event) => {
              const { requestId, code, language, theme } = event.detail;
              ...
              try {
                ...
                const html = window.__shikiHighlighter.codeToHtml(code, {
                  lang: language || 'plaintext',
                  theme: theme || 'github-dark'
                });
                
                window.dispatchEvent(new CustomEvent('shiki-highlight-result', {
                  detail: { requestId, success: true, html }
                }));
                
              } catch (error) {
                console.error('Main world: Shiki highlighting failed:', error);
                const errorMessage = error instanceof Error ? error.message : 'Unknown highlighting error';
                window.dispatchEvent(new CustomEvent('shiki-highlight-error', {
                  detail: { requestId, error: errorMessage }
                }));
              }
            });

            // Notify content script that Shiki is ready
            window.dispatchEvent(new CustomEvent('shiki-ready'));
            
          } catch (error) {
            console.error('Main world: Failed to initialize Shiki:', error);
            const errorMessage = error instanceof Error ? error.message : 'Unknown initialization error';
            window.dispatchEvent(new CustomEvent('shiki-error', { 
      ...
      script.onerror = (error) => {
        console.error('Main world: Failed to load Shiki script:', error)
        window.dispatchEvent(
          new CustomEvent('shiki-error', {
            detail: { error: 'Failed to load Shiki script from CDN' },
          }),
        )
      }
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === 'initializeShiki' && sender.tab?.id) {
    injectShiki(sender.tab.id)
      .then(() => {
        sendResponse({ success: true })
      })
      .catch((error) => {
        console.error('Failed to inject Shiki:', error)
