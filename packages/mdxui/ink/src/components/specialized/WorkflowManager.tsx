...
import { createWorkflowFromFrontmatter, executeWorkflowStep } from '../../workflow/manager.js'
import type { WorkflowFrontmatter } from '../../workflow/types.js'
import { InputForm } from '../interactive/InputForm.js'
...
export const WorkflowManager: React.FC<WorkflowManagerProps> = ({ frontmatter, onComplete, onCancel }) => {
  ...
  return (
    <Box flexDirection='column'>
      ...
      <WorkflowProgress currentStep={currentStepIndex + 1} totalSteps={workflow.steps.length} />
    </Box>
  )
}
...
const WorkflowHeader: React.FC<WorkflowHeaderProps> = ({ workflow, currentStep, totalSteps }) => {
  return (
...
const StepDisplay: React.FC<StepDisplayProps> = ({ step, stepNumber, totalSteps, onSubmit, onBack, onCancel }) => {
  return (
    <Box flexDirection='column' padding={1}>
      ...
      <InputForm frontmatter={{ inputs: step.inputSchema?.shape || {} }} onSubmit={onSubmit} />
...
const WorkflowProgress: React.FC<WorkflowProgressProps> = ({ currentStep, totalSteps }) => {
