...
import { Ascii } from '../base/Ascii.js'
 ...
 */
export const BaseSection: React.FC<{ children: React.ReactNode; title?: string; badge?: string }> = ({ children, title, badge }) => {
  return (
 ...
 */
export const Problem: React.FC<ProblemSection> = ({ headline, description, points, badge }) => {
  return (
 ...
 */
const FeatureItem: React.FC<FeatureItemType> = ({ title, description }) => {
  return (
 ...
 */
export const Features: React.FC<FeaturesSection> = ({ headline, description, features, badge, primaryActionText, primaryActionLink }) => {
  return (
    <BaseSection title={headline} badge={badge}>
      {description && (
        <Box marginBottom={1}>
 ...
 */
const PricingPlanComponent: React.FC<PricingPlan> = ({ name, price, description, features, ctaText, ctaLink, featured }) => {
  const borderColor = featured ? 'green' : 'blue'
  const nameColor = featured ? 'green' : 'blue'

  return (
    <Box flexDirection='column' borderStyle='round' borderColor={borderColor} padding={1} marginRight={2} marginBottom={1} width={30}>
 ...
 */
export const Pricing: React.FC<PricingSection> = ({ headline, description, plans, note, badge }) => {
  return (
    <BaseSection title={headline} badge={badge}>
      {description && (
        <Box marginBottom={1}>
 ...
 */
const TestimonialComponent: React.FC<Testimonial> = ({ quote, author, authorTitle }) => {
  return (
    <Box flexDirection='column' borderStyle='round' borderColor='blue' padding={1} marginRight={2} marginBottom={1} width={40}>
 ...
 */
export const Testimonials: React.FC<TestimonialsSection> = ({ headline, description, testimonials, badge }) => {
  return (
    <BaseSection title={headline} badge={badge}>
      {description && (
        <Box marginBottom={1}>
 ...
 */
export const FAQ: React.FC<FAQSection> = ({ headline, description, faqs, badge }) => {
  return (
    <BaseSection title={headline} badge={badge}>
      {description && (
        <Box marginBottom={1}>
...
export const CallToAction: React.FC<CallToActionSection> = ({
  ...
  return (
    <Box flexDirection='column' padding={1} borderStyle='double' borderColor='green'>
      {badge && <Text color='yellow'>{badge}</Text>}
      <Ascii font='doom'>{headline}</Ascii>
      {description && (
        <Box marginY={1}>
          <Markdown>{description}</Markdown>
        </Box>
 ...
 */
const TeamMemberComponent: React.FC<TeamMember> = ({ name, role, bio }) => {
  return (
    <Box flexDirection='column' borderStyle='round' borderColor='blue' padding={1} marginRight={2} marginBottom={1} width={30}>
      <Text bold>{name}</Text>
 ...
 */
export const Team: React.FC<TeamSection> = ({ headline, description, members, badge }) => {
  return (
    <BaseSection title={headline} badge={badge}>
      {description && (
        <Box marginBottom={1}>
