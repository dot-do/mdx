...
import { mergeComponents } from '../../component-system/loader.js'
import { getDefaultComponents } from '../../components/base/index.js'
import type { MDXComponents } from '../../component-system/loader.js'
import { executeCodeBlocks, type CodeExecutionResult } from '../../utils/code-execution.js'
 ...
 */
export function InkMDXRenderer({ file, content, Component, components, scope, executeCode = false }: InkMDXRendererProps) {
  const [mdxComponent, setMdxComponent] = useState<React.ComponentType<any> | null>(Component || null)
  const [error, setError] = useState<Error | null>(null)
  const [loading, setLoading] = useState<boolean>(!Component && (!!file || !!content))
  const [executionResults, setExecutionResults] = useState<any[]>([])

  const mergedComponents = mergeComponents(getDefaultComponents(), components)
  ...
  useEffect(() => {
    ...
    async function loadMdx() {
      try {
        ...
        const { compileMdx } = await import('../../core/compiler.js')
        const { getAllComponents } = await import('../../component-system/loader.js')
  ...
  return (
    <Box flexDirection='column'>
      <MdxComponent />
      {executeCode && executionResults.length > 0 && <ExecutionResults results={executionResults} />}
    </Box>
  )
}
