...
import { z } from 'zod'
import { MdxFrontmatter } from '../../utils/types.js'
import { createSchemaFromFrontmatter } from '../../core/schema.js'
...
const InputFormFields: React.FC<InputFormFieldsProps> = ({ fields, schema, onSubmit }) => {
  ...
  return (
    <Box flexDirection='column' padding={1} borderStyle='round' borderColor='blue'>
      ...
      {fields.map(([name, config], index) => (
        <Box key={name} flexDirection='column' marginY={1}>
          ...
          {currentFieldIndex === index && (
            <Box marginLeft={2}>
              <InputField name={name} config={config} value={values[name]} onChange={(value) => setValues((prev) => ({ ...prev, [name]: value }))} />
            </Box>
          )}
      ...
      <Box marginTop={1}>
        <Text dimColor>
          Press <Text color='green'>Enter</Text> to {isLastField ? 'submit' : 'continue'},<Text color='yellow'> ↑/↓</Text> to navigate,
