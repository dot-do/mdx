import Indicator, { type Props as IndicatorProps } from '../base/Indicator.js'
import ItemComponent, { type Props as ItemProps } from '../base/Item.js'
...
function SelectInput<V>({
  ...
  useEffect(() => {
    if (
      !isEqual(
        previousItems.current.map((item: Item<V>) => item.value),
        items.map((item: Item<V>) => item.value),
      )
    ) {
      setRotateIndex(0)
      setSelectedIndex(0)
    ...
    useCallback(
      (input: string, key: any) => {
        if (input === 'k' || key.upArrow) {
          ...
          const slicedItems = hasLimit ? arrayToRotated(items, nextRotateIndex).slice(0, limit) : items
