...
const EventProcessor: React.FC = () => {
  ...
  const totalEvents = events.reduce((count: number, event: any) => {
    const childCount = event.children ? event.children.length : 0
    return count + 1 + childCount
  }, 0)

  const completedEvents = events.reduce((count: number, event: any) => {
    const isCompleted = event.status === 'completed' || event.status === 'failed'
    const completedChildren = event.children ? event.children.filter((child: any) => child.status === 'completed' || child.status === 'failed').length : 0
  ...
  return (
    <Box flexDirection='column' padding={1}>
      ...
      <EventProgressIndicator current={completedEvents} total={totalEvents} />

      <EventStatus events={events} title='Event Processing Status' />
    </Box>
  )
}
