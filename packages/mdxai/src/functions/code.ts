...
import { validateCode, type ValidationResult } from '../../../mdxe/src/test'
...
export const code = createUnifiedFunction<Promise<CodeResult>>((content: string, options: Record<string, any>) => {
 ...
 */
export const codeWithValidation = createUnifiedFunction<Promise<CodeResult>>(async (content: string, options: Record<string, any>) => {
  const codeResult = await code(content, options)

  // Validate the generated code using the new @mdxe/test package
  const validation = await validateCode(codeResult.code, codeResult.tests, { runTests: true })
