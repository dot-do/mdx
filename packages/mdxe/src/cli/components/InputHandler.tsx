...
export const InputHandler: React.FC<InputHandlerProps> = ({ frontmatter, onInputsCollected, initialInputs = {} }) => {
  ...
  useEffect(() => {
    if (frontmatter.inputs && Object.keys(frontmatter.inputs).length > 0) {
      ...
      const missingRequiredInputs = requiredInputs.filter((name) => inputs[name] === undefined || inputs[name] === null || inputs[name] === '')
  ...
  return (
    <Box flexDirection='column'>
      <InputForm frontmatter={frontmatter} onSubmit={handleInputSubmit} />
    </Box>
  )
}
